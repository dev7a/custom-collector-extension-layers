...
from scripts.otel_layer_utils.distribution_utils import (
    load_distributions, DistributionError, resolve_build_tags
)
from scripts.otel_layer_utils.ui_utils import (
    header, subheader, status, info, detail, 
    success, error, warning, 
    spinner, format_table, StepTracker,
    set_verbose_mode, debug
)
...
def is_python_script(cmd):
    ...
    return (cmd[0].endswith('python') or cmd[0].endswith('python3')) and cmd[1].endswith('.py')

def run_command(cmd, cwd=None, env=None, check=True, capture_output=False, capture_github_env=False, use_spinner=False):
    ...
    cmd_type = "python script" if is_python else "external command"
    ...
    if capture_github_env:
        ...
        for file_path, file_type in [(github_env_path, "GITHUB_ENV"), (github_output_path, "GITHUB_OUTPUT")]:
            try:
...
def check_aws_credentials():
    ...
    try:
        ...
        if account_id:
            ...
            warning("AWS credentials are configured but account ID couldn't be determined.")
            return False
...
def get_aws_region():
    ...
    try:
        ...
        if region:
            ...
            detail("Hint", "Configure region via AWS_REGION env var or 'aws configure'.")
            sys.exit(1)
    except Exception as e:
...
try:
    ...
    if not DISTRIBUTION_CHOICES or not _distributions_data:
     ...
     error("Fatal Error: Could not import distribution_utils", "Check project structure and dependencies.")
     sys.exit(1)
    ...
    error(f"Fatal Error: Could not load distributions from config file {_dist_yaml_path}", str(e))
    sys.exit(1)
...
def main(distribution, architecture, upstream_repo, upstream_ref, layer_name, runtimes, skip_publish, verbose, public, keep_temp):
    ...
    try:
        ...
        run_command(
            ["git", "clone", "--depth", "1", "--branch", upstream_ref, repo_url, str(temp_upstream_path)],
            use_spinner=True
        )
        ...
        try:
            ...
            error(f"Error resolving build tags for distribution '{distribution}'", str(e))
        ...
        if not layer_arn:
             ...
             arn_match = re.search(r'Published Layer ARN: (arn:aws:lambda:[^:]+:[^:]+:layer:[^:]+:[0-9]+)', publish_result.stdout)
             if arn_match:
        ...
        success(f"Published {distribution} distribution to region {region} as a 'local' release")
        info("Next steps", "You can now test this layer by attaching it to a Lambda function")
...
if __name__ == "__main__":
    ...
    try:
        ...
        detail("Hint", "pip install -r requirements-dev.txt or pip install pyyaml click boto3")
        sys.exit(1)
    main()
