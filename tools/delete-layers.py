...
from scripts.otel_layer_utils.ui_utils import (
    header, subheader, status, info, success, error, warning, 
                    ...
                    for layer in page['Layers']:
                        ...
                        if fnmatch.fnmatch(layer_name, pattern):
                            ...
                            try:
                                ...
                                sp.fail("✗")
                                error(f"Error getting versions for layer {layer_name}", str(e))
...
def delete_layers(layers: List[Dict], dry_run: bool = False, skip_dynamodb: bool = False) -> Tuple[int, int, int, int]:
    """
    ...
    for idx, layer in enumerate(layers):
        ...
        if dry_run:
            info("[DRY RUN]", f"Would delete layer {layer_name} in {region} with {len(versions)} version(s)")
            lambda_success += len(versions)
            
            if not skip_dynamodb:
                info("[DRY RUN]", f"Would delete DynamoDB records for {len(versions)} version(s)")
        ...
        try:
            ...
            for version in versions:
                ...
                try:
                    with yaspin(text=f"Deleting {layer_name} version {version_number} in {region}...") as sp:
                    ...
                    if not skip_dynamodb:
                        with yaspin(text=f"Deleting DynamoDB record for {layer_arn}...") as sp:
                    ...
                    sp.fail("✗")
                    error(f"Error deleting {layer_name} version {version_number} in {region}", str(e))
                    lambda_failure += 1
                    versions_failed += 1
            
            # Update step tracker with a better formatted result message
            has_failures = versions_failed > 0 or (not skip_dynamodb and dynamo_versions_failed > 0)
            
            # Format a multi-line status message for better readability
            status_lines = []
            status_lines.append(f"Lambda Layer: {versions_success} deleted{f', {versions_failed} failed' if versions_failed > 0 else ''}")
            
            if not skip_dynamodb:
                status_lines.append(f"DynamoDB Records: {dynamo_versions_success} deleted{f', {dynamo_versions_failed} failed' if dynamo_versions_failed > 0 else ''}")
...
def print_layer_summary(layers: List[Dict]):
    ...
    status("Found", f"{total_layers} layer(s) with a total of {total_versions} version(s)")
...
def confirm_deletion(layers: List[Dict], force: bool = False, skip_dynamodb: bool = False) -> bool:
    """
    ...
    warning("Warning", f"You are about to delete {len(layers)} layer(s) with {total_versions} total version(s).")
...
def main():
    ...
    if args.dry_run:
        ...
        if not args.skip_dynamodb:
            info("Dry Run Summary", f"Would have deleted {dynamo_success} DynamoDB record(s)")
    else:
        if lambda_success > 0:
