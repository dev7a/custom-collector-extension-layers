...
from otel_layer_utils.distribution_utils import load_distributions, resolve_build_tags, DistributionError

# Get the GitHub Actions output file path. Fail if not set.
GITHUB_OUTPUT_FILE = os.environ.get("GITHUB_OUTPUT")
if not GITHUB_OUTPUT_FILE:
    print("Error: GITHUB_OUTPUT environment variable not set. This script must run in GitHub Actions.", file=sys.stderr)
    sys.exit(1)
...
def set_github_output(name, value):
    ...
    try:
        ...
        print(f"Error writing to GITHUB_OUTPUT file {GITHUB_OUTPUT_FILE}: {e}", file=sys.stderr)
...
if not collector_version:
     print("Error: INPUT_COLLECTOR_VERSION environment variable not set.", file=sys.stderr)
     sys.exit(1)
