...
def status(text: str, value: str) -> None:
    ...
    Args:
        ...
        value: Status value
    """
    click.secho(f"{STYLE_CONFIG['status_prefix']}{text}", 
                fg=COLORS["subheader"], nl=False)
...
def detail(text: str, value: str) -> None:
    ...
    Args:
        ...
        value: Detail value
    """
    click.secho(f"{STYLE_CONFIG['detail_prefix']}{text}", 
                fg=COLORS["info"], nl=False)
    click.echo(f"{STYLE_CONFIG['separator']}{value}")
...
def success(text: str, value: Optional[str] = None) -> None:
    ...
    """
    click.secho(f"{STYLE_CONFIG['success_prefix']}{text}", 
                fg=COLORS["success"], nl=False)
    if value:
        click.echo(f"{STYLE_CONFIG['separator']}{value}")
    else:
        click.echo("")
...
def error(text: str, details: Optional[str] = None) -> None:
    ...
    """
    click.secho(f"{STYLE_CONFIG['error_prefix']}{text}", 
                fg=COLORS["error"], bold=True, nl=False)
...
def warning(text: str, details: Optional[str] = None) -> None:
    ...
    """
    click.secho(f"{STYLE_CONFIG['status_prefix']}Warning", 
                fg=COLORS["warning"], nl=False)
    click.echo(f"{STYLE_CONFIG['separator']}{text}")
    if details:
        click.secho(f"{STYLE_CONFIG['detail_prefix']}Detail", 
                    fg=COLORS["info"], nl=False)
...
def format_table(headers: List[str], rows: List[List[str]], title: Optional[str] = None) -> None:
    ...
    # Print headers
    header_row = STYLE_CONFIG["table"]["column_separator"].join(str(h).ljust(w) for h, w in zip(headers, col_widths))
    ...
    # Print data rows
    for row in rows:
        data_row = STYLE_CONFIG["table"]["column_separator"].join(str(c).ljust(w) for c, w in zip(row, col_widths))
        click.secho(f"{STYLE_CONFIG['detail_prefix']}{data_row}", fg=COLORS["info"])


def progress_bar(current: int, total: int, width: int = None, prefix: str = "", suffix: str = "") -> None:
...
class StepTracker:
    ...
    def __init__(self, steps: List[str], title: Optional[str] = None):
        ...
        self.steps = steps
        self.title = title
        self.status = ["pending"] * len(steps)  # "pending", "running", "complete", "failed"
    ...
    def _render(self):
        ...
        if self.title:
            click.echo()
            click.secho(f"{STYLE_CONFIG['header_prefix']}{_format_heading_case(self.title)}", 
                      fg=COLORS["header"], bold=True)
        ...
        for i, step in enumerate(self.steps):
            ...
            click.secho(f"{STYLE_CONFIG['detail_prefix']}{symbol} {step}{message}", 
